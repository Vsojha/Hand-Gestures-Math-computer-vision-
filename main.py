...
genai.configure(api_key="AIzaSyDFjUwDlfWKLg32nyLywBEn_gs4f3Pb2n0")
...
def getHandInfo(img):
    ...
    hands, img = detector.findHands(img,False,True)  # with draw
...
def draw(info,prev_pos,canvas):
    ...
    if fingers==[0,1,0,0,0]:
        current_pos=lmList[8][0:2] #8 because it is predefined by developer can be searched on google
...
while True:
    ...
    img=cv2.flip(img,1)
    if canvas is None:
        canvas=np.zeros_like(img)#black image
